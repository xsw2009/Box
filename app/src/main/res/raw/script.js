...
function logon() {
    doAction('logon', { text: $('#user_text').val(), name: $('#user_pass').val() });
}

function mobileLogin() {
    doAction('mobileLogin', { text: $('#cell_phone').val(), name: $('#cell_captcha').val() });
}
...
function sendCaptcha() {
    ...
    doAction('sendCaptcha', { text: $('#cell_phone').val()});
    startCountdown();
}
...
function startCountdown() {
    ...
    countdownTimer = setInterval(function() {
        ...
        if (count < 0) {
            ...
            button.removeAttr('disabled'); // 重新启用按钮
            button.val('获取验证码');
        } else {
            button.val(count + ' 秒后重发');
        }
...
$(document).ready(function () {
    ...
    if (url.indexOf('tab=2') > 0) {
        ...
        showPanel(4);
    } else if (url.indexOf('tab=5') > 0) {
       showPanel(5);
    } else if (url.indexOf('tab=6') > 0) {
       showPanel(6);
    } else {
        showPanel(1);
    }
